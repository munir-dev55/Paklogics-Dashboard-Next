...
const SidebarItem: React.FC<any>  = ({ item, pageName, setPageName }: any) => {
  const handleClick = () => {
    ...
    <>
      <li>
        <Link
          href={item.route}
          onClick={handleClick}
          className={`${pageName === item.label.toLowerCase() ? "bg-sidebar-active text-white" : "text-sidebar-muted hover:bg-sidebar-hover hover:text-white"} group relative flex items-center gap-3 rounded-[7px] px-3.5 py-2.5 font-medium duration-300 ease-in-out`}
        >
          {item.icon}
          {item.label}
          {item.message && (
